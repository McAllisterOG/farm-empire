...
describe('First Farmhand workforce', () => {
  ...
  it('migrates v13 closed, fails malformed progression closed, and round-trips a valid hire', () => {
    ...
    expect(farmOf(migrated).workforce).toMatchObject({ farmhandHired: false, lastShiftPaidDay: 0, eliotHired: false, dispatchApprovedDay: 0, manager: { hired: false, enabled: false, parcelId: 'starter', cropId: 'crop_corn', lastReviewedDay: 0 } });
    ...
    expect(farmOf(safe).workforce).toMatchObject({ farmhandHired: false, lastShiftPaidDay: 0, eliotHired: false, dispatchApprovedDay: 0, manager: { hired: false, enabled: false, parcelId: 'starter', cropId: 'crop_corn', lastReviewedDay: 0 } });

    const hired = hiredFarm();
    farmOf(hired).workforce.lastShiftPaidDay = 1;
    const reloaded = deserialize(serialize(hired, NOW + 5), NOW + 6);
    expect(farmOf(reloaded).workforce).toMatchObject({ farmhandHired: true, lastShiftPaidDay: 1, eliotHired: false, dispatchApprovedDay: 0, manager: { hired: false, enabled: false, parcelId: 'starter', cropId: 'crop_corn', lastReviewedDay: 0 } });
