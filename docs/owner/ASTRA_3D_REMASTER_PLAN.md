...
QA uses fresh local origin `http://127.0.0.1:5193` after detecting existing saves on port 5178; those saves were not opened or modified. Restore rehearsal is served separately on port 5194. Evidence lives under ignored `release/astra-qa`; no owner save is a repository artifact. Stages 1–5 are implemented and locally verified. See ASTRA_3D_REMASTER_VERIFICATION.md for exact evidence and limits. Stage 6 awaits the independent Owner Console review gate before normal publication.
