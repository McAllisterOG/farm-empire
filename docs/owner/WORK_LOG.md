...
## Demo-Complete V1 release regression - incomplete host acceptance
...
- **Blocker/boundary:** Isolated Electron player-surface acceptance could not run on this host: its renderer exits during GPU-process launch with Windows exit `-1073741515`, including with `--disable-gpu`, before any page/CDP target exists. The profile/logs were removed. No owner save/profile, Desktop shortcut, deployment, publication, gameplay, save authority, or product scope changed. Browser acceptance must run elsewhere before release acceptance, Red Team, shortcut work, or publication.
