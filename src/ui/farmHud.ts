...
export class FarmHud {
  ...
  update(state: GameState, runtime?: TractorHudRuntime): void {
    ...
    this.storageEl.textContent = `${formatFarmCargoWeight(storageUsed(state))} / ${formatFarmCargoWeight(farm.storageCapacity)} · P ${formatFarmCargoWeight(pickupCargoUsed(state))} / ${formatFarmCargoWeight(pickupCargoCapacity(state))}`;
    ...
    this.harvestDestinationButton.title = `Click to switch where manual harvest baskets are unloaded. Basket: ${formatFarmCargoWeight(basketUsed)} / ${formatFarmCargoWeight(HAND_BASKET_CAPACITY)}.`;
    this.unloadBasketButton.classList.toggle('hidden', basketUsed <= 0);
    this.unloadBasketButton.textContent = `Unload Basket · ${formatFarmCargoWeight(basketUsed)}`;
